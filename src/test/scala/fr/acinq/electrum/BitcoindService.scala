...
trait BitcoindService extends Logging {
  ...
  def startBitcoind(): Unit = {
    ...
    if (!Files.exists(new File(PATH_BITCOIND_DATADIR.toString, "bitcoin.conf").toPath)) {
      Files.copy(classOf[BitcoindService].getResourceAsStream("/bitcoin.conf"), new File(PATH_BITCOIND_DATADIR.toString, "bitcoin.conf").toPath, StandardCopyOption.REPLACE_EXISTING)
    }
