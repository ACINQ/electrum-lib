...
import fr.acinq.electrum.BitcoinCoreWallet.{FundTransactionResponse, SignTransactionResponse}
import fr.acinq.electrum.ElectrumClient.{BroadcastTransaction, BroadcastTransactionResponse}
...
class ElectrumWalletSpec extends TestKit(ActorSystem("test")) with FunSuiteLike with BitcoindService with ElectrumxService  with BeforeAndAfterAll with Logging {
  ...
  test("handle transactions with identical outputs to us") {
    ...
    val tx = Transaction(version = 2,
      txIn = Nil,
      txOut = Seq(
        TxOut(amount, addressToPublicKeyScript(address, Block.RegtestGenesisBlock.hash)),
        TxOut(amount, addressToPublicKeyScript(address, Block.RegtestGenesisBlock.hash))
      ), lockTime = 0L)
  ...
  test("send money to someone else (we broadcast)") {
    ...
    val tx = Transaction(version = 2, txIn = Nil, txOut = TxOut(Btc(1), addressToPublicKeyScript(address, Block.RegtestGenesisBlock.hash)) :: Nil, lockTime = 0L)
