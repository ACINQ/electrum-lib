...
import fr.acinq.electrum.ElectrumClient.{GetTransaction, GetTransactionResponse, TransactionHistoryItem}
...
object ElectrumWallet {
  ...
  case class Data(chainHash: BinaryData,
    ...
    def completeTransaction(tx: Transaction, feeRatePerKw: Long, minimumFee: Satoshi, dustLimit: Satoshi, allowSpendUnconfirmed: Boolean): (Data, Transaction, Satoshi) = {
      ...
      val utxos = chooseUtxos(amount + weight2fee(feeRatePerKw, 1000), allowSpendUnconfirmed)
      ...
      val fee1 = {
        ...
        weight2fee(feeRatePerKw, tx2.weight())
      }
    ...
    def commitTransaction(tx: Transaction): Data = {
      ...
      val history1 = (tx.txIn.filter(isMine).map(extractPubKeySpentFrom).flatten.map(computeScriptHashFromPublicKey) ++ tx.txOut.filter(isMine).map(_.publicKeyScript).map(ElectrumClient.computeScriptHash))
    ...
    def spendAll(publicKeyScript: BinaryData, feeRatePerKw: Long) : (Transaction, Satoshi) = {
      ...
      val tx1 = addUtxosWithDummySig(tx, utxos)
      val fee = weight2fee(feeRatePerKw, tx1.weight())
